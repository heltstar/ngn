...
int main(int argc, char **argv)
{
    ...
    for ( ;; ){
        ...
        if (((long int)rows) != 0){
            ...
            memset(req_record, 0, sizeof(cfs_req_t));
            req_record->file_path = strdup(result_row[1]);

            cfs_log(NOTICE, "first try to get resources(file_path=%s) from other cfs nodes...",req_record->file_path);
            printf("first try to get resources(file_path=%s) from other cfs node \n",req_record->file_path);
            cfs_cfsedge_config_t *pccc = g_config->cfsedge;// first try to get resources from other cfs nodes.
            while(pccc != NULL)//
            {	
                printf("while loop.\n");
                req_record->req_port = pccc->port;
                req_record->req_host = strdup(pccc->host);
                req_record->req_path = strdup(req_record->file_path);

                printf("try to get file(file_path=%s) from cfsnode host:port is %s:%d\n",req_record->file_path, pccc->host, pccc->port);
                cfs_http_header_t header = {0};
                int nres = my_cfs_get_header(req_record, &header);
                if(nres == 0)
                {
                    printf("[\npath=%s\nsize=%lld\nhost=%s\napi=%s\nhhttp=%d\nhttp=%d\n]\n", 
                            req_record->file_path, header.content_length, 
                            req_record->req_host, req_record->req_path, 
                            header.hdfs_code, header.http_code);

                    if(header.http_code == 200)
                    {
                        req_record->file_size = header.content_length;
                        cfs_log(NOTICE, "cfs_download start");
                        printf("cfs_download start\n");
                        int flag = my_cfs_download(req_record, g_config->work_dir);
                        if(flag == -1)
                        {
                            break;
                        }
...
cfs_download(cfs_req_t *req_info, char *work_path)
{
    ...
    static int
my_cfs_download(cfs_req_t *req_info, char *work_path)
{
    char *dir_buff                        = NULL;
    char tmp_name[256]                    = {0};
    char dir[256]                         = {0};
...
static int http_cmd_get(int sock, char *buf)
{
    ...
    char   *head_buff    = NULL;
    char   *rh           = NULL;
    http_header_server_t *header;
    const char *split    = "\r\n\r\n";

    rh = (char *)cfs_malloc(1024);
    header = (http_header_server_t*)cfs_malloc(sizeof(http_header_server_t));
    head_buff = (char *)cfs_malloc(1024);
